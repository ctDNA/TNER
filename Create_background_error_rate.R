...
# Create a background error rate file from individual healthy subject pileup data
...
#          The columns are CHR = chromosome number; POS= genomic coordinate position
...
hs.files=list.files(path=data.dir,pattern="freq$") # list all healthy subject pileup data files
...
for (i in 1:length(hs.files))
{
 ...
 # calculate background error rate
  ...
  if (i==1) hs.depth=pileup.data$DEPTH else hs.depth=cbind(hs.depth,pileup.data$DEPTH) #save the depth 
}

#combine all background error rate into an array
hsa=array(unlist(hs.db),c(dim(hs.db[[1]]),length(hs.db)),dimnames = list(pileup.data$POS,s,hs.files))
hs.bkg.ave=apply( hsa,1:2,mean )   #calculate the average background error rate

rownames(hs.bkg.ave)=rownames(hs.depth)=pileup.data$POS
colnames(hs.bkg.ave)=s
